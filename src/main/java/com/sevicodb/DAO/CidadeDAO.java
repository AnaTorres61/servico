...
public class CidadeDAO extends ConexaoDB {
    ...
    public void insertCidade(Cidade entidade) {
        try (PreparedStatement preparedStatement = prapararSQL(INSERT_CIDADE_SQL)) {
    ...
    public Cidade selectCidade(int id) {
        ...
        try (PreparedStatement preparedStatement = prapararSQL(SELECT_CIDADE_BY_ID)) {
            ...
            while (rs.next()) {
                ...
                Integer id_uf = rs.getInt("id_uf");
                entidade = new Cidade(id, descricao, codigo, id_uf);
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
    ...
    public List<Cidade> selectAllCidades() {
        ...
        try (PreparedStatement preparedStatement = prapararSQL(SELECT_ALL_CIDADE)) {
            ResultSet rs = preparedStatement.executeQuery();
            ...
            while (rs.next()) {
                ...
                Integer codigo = rs.getInt("codigo");
                Integer id_uf = rs.getInt("id_uf");
                entidades.add(new Cidade(id, descricao, codigo, id_uf));
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
    ...
    public boolean deleteCidade(int id) throws SQLException {
        try (PreparedStatement statement = prapararSQL(DELETE_CIDADE_SQL)) {
    ...
    public boolean updateCidade(Cidade entidade) throws SQLException {
        try (PreparedStatement statement = prapararSQL(UPDATE_CIDADE_SQL)) {
            statement.setString(1, entidade.getDescricao());
            statement.setInt(2, entidade.getCodigo());
            statement.setInt(3, entidade.getId_uf());
